...
export interface QCSidewalkPermitApplicationProps {
  onCancel: () => void;
  onSubmit: (data: {
    ...
    applyingFor: string;
    isAlongNationalRoad: string;
  }) => void;
  showToast?: (msg: string) => void;
}

export const QCSidewalkPermitApplication: React.FC<QCSidewalkPermitApplicationProps> = ({
  onCancel,
  onSubmit,
  showToast
}) => {
  ...
  const [applicantLastName, setApplicantLastName] = useState<string>('Ramos');
  const [applicantFirstName, setApplicantFirstName] = useState<string>('Gabriel');
  const [applicantMI, setApplicantMI] = useState<string>('M.');
  const [mobileNo, setMobileNo] = useState<string>('0918 222 6655');
  const [applicantAddress, setApplicantAddress] = useState<string>('Timog Avenue cor. Tomas Morato Ave., South Triangle, Quezon City');

  // Panel 2: Lot Location
  const [lotNo, setLotNo] = useState<string>('2');
  const [blkNo, setBlkNo] = useState<string>('14');
  const [tctNo, setTctNo] = useState<string>('TCT-004-2023001928');
  const [taxDecNo, setTaxDecNo] = useState<string>('TD-E-041-00291-QC');
  const [street, setStreet] = useState<string>('Timog Avenue cor. Tomas Morato Ave.');
  const [barangay, setBarangay] = useState<string>('South Triangle');
  ...
  // Panel 3: Specific Questions
  const [applyingFor, setApplyingFor] = useState<string>('Sidewalk Enclosure');
  const [isAlongNationalRoad, setIsAlongNationalRoad] = useState<string>('No');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (showToast) {
      showToast('Sidewalk Permit application submitted successfully!');
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-4">
      <div className="rounded-xl overflow-hidden border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#0c1629] shadow-xs">
        {/* Top Header bar matching Picture 4 */}
        ...
        <div className="p-4 sm:p-5">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* ============================================================== */}
            {/* PANEL 3: SIDEWALK SPECIFIC QUESTIONS */}
            {/* ============================================================== */}
            <div className="p-4 sm:p-5 rounded-lg border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[#0b1322] space-y-4 shadow-2xs">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-6">
                  <label className="inline-flex items-center space-x-2 cursor-pointer text-xs text-slate-700 dark:text-slate-300">
                    <input
                      ...
                      name="applyingFor"
                      value="Street Canopy"
                      checked={applyingFor === 'Street Canopy'}
                      onChange={() => setApplyingFor('Street Canopy')}
                      className="w-4 h-4 text-[#0052cc] focus:ring-[#0052cc] cursor-pointer"
                    />
                    <span>Street Canopy</span>
                  </label>
                  <label className="inline-flex items-center space-x-2 cursor-pointer text-xs text-slate-700 dark:text-slate-300">
                    <input
                      type="radio"
                      name="applyingFor"
                      value="Sidewalk Enclosure"
                      checked={applyingFor === 'Sidewalk Enclosure'}
                      onChange={() => setApplyingFor('Sidewalk Enclosure')}
                      className="w-4 h-4 text-[#0052cc] focus:ring-[#0052cc] cursor-pointer"
                    />
                    <span>Sidewalk Enclosure</span>
                  </label>
                </div>
              </div>

              {/* Question 2: Is the project location along National Road or Highway * */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                <label className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200">
                  Is the project location along National Road or Highway <span className="text-red-500 font-bold">*</span>
                </label>
                <div className="flex items-center space-x-6">
                  <label className="inline-flex items-center space-x-2 cursor-pointer text-xs text-slate-700 dark:text-slate-300">
                    <input
                      type="radio"
                      name="isAlongNationalRoad"
                      value="Yes"
                      checked={isAlongNationalRoad === 'Yes'}
                      onChange={() => setIsAlongNationalRoad('Yes')}
                      className="w-4 h-4 text-[#0052cc] focus:ring-[#0052cc] cursor-pointer"
                    />
                    <span>Yes</span>
                  </label>
                  <label className="inline-flex items-center space-x-2 cursor-pointer text-xs text-slate-700 dark:text-slate-300">
                    <input
                      ...
                      name="isAlongNationalRoad"
                      value="No"
                      checked={isAlongNationalRoad === 'No'}
                      onChange={() => setIsAlongNationalRoad('No')}
                      className="w-4 h-4 text-[#0052cc] focus:ring-[#0052cc] cursor-pointer"
                    />
                    <span>No</span>
                  </label>
