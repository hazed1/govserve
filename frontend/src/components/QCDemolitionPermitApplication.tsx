...
export interface QCDemolitionPermitApplicationProps {
  onCancel: () => void;
  onSubmit: (data: {
...
export const QCDemolitionPermitApplication: React.FC<QCDemolitionPermitApplicationProps> = ({
  onCancel,
  onSubmit,
  showToast
}) => {
  // Panel 1: Ownership Information
  ...
  const [address, setAddress] = useState<string>('14 Jasmine St., Fairview Park Subd., Quezon City');

  // Panel 2: Lot Location
  const [lotNo, setLotNo] = useState<string>('5');
  const [blkNo, setBlkNo] = useState<string>('12');
  const [tctNo, setTctNo] = useState<string>('TCT-004-2018009214');
  const [taxDecNo, setTaxDecNo] = useState<string>('TD-E-032-11928-QC');
  const [street, setStreet] = useState<string>('88 Aurora Boulevard cor. Harvard Street');
  const [barangay, setBarangay] = useState<string>('San Martin de Porres');
  const [district, setDistrict] = useState<string>('District 3');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (showToast) {
      showToast('Demolition Permit application submitted successfully!');
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-4">
      <div className="rounded-xl overflow-hidden border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#0c1629] shadow-xs">
        {/* Top Header bar matching Picture 3 */}
        ...
        <div className="p-4 sm:p-5">
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* ============================================================== */}
            {/* PANEL 1: OWNERSHIP INFORMATION (WITH SUB-HEADER BANNER) */}
            {/* ============================================================== */}
            <div className="rounded-lg border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[#0b1322] shadow-2xs overflow-hidden">
              {/* Header bar with white background and readable text */}
              <div className="bg-white dark:bg-[#0b1322] text-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-700/80 px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider">
