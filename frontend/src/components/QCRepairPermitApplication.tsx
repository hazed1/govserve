...
export interface QCRepairPermitApplicationProps {
  onCancel: () => void;
  onSubmit: (data: {
...
export const QCRepairPermitApplication: React.FC<QCRepairPermitApplicationProps> = ({
  onCancel,
  onSubmit,
  showToast
}) => {
  ...
  const [applicantAddress, setApplicantAddress] = useState<string>('Aurora Boulevard cor. Cambridge Street, Cubao, Quezon City');

  // Panel 2: Lot Location
  const [lotNo, setLotNo] = useState<string>('9');
  const [blkNo, setBlkNo] = useState<string>('3');
  const [tctNo, setTctNo] = useState<string>('TCT-004-2015004912');
  const [taxDecNo, setTaxDecNo] = useState<string>('TD-E-033-88192-QC');
  const [street, setStreet] = useState<string>('Aurora Boulevard cor. Cambridge Street');
  const [barangay, setBarangay] = useState<string>('Cubao');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (showToast) {
      showToast('Repair Permit application submitted successfully!');
    }
  };
