...
export const QCExcavationGroundPermitApplication: React.FC<QCExcavationGroundPermitApplicationProps> = ({
  ...
  const [applicantLastName, setApplicantLastName] = useState<string>('Gomez');
  const [applicantFirstName, setApplicantFirstName] = useState<string>('Leopoldo');
  const [applicantMI, setApplicantMI] = useState<string>('M.');
  const [mobileNo, setMobileNo] = useState<string>('0917 999 4433');
  const [applicantAddress, setApplicantAddress] = useState<string>('Kalayaan Avenue cor. Elliptical Road, Central, Quezon City');

  // Panel 2: Lot Location
  const [lotNo, setLotNo] = useState<string>('10-A');
  const [blkNo, setBlkNo] = useState<string>('5');
  const [tctNo, setTctNo] = useState<string>('TCT-004-2024008129');
  const [taxDecNo, setTaxDecNo] = useState<string>('TD-E-044-01928-QC');
  const [street, setStreet] = useState<string>('Kalayaan Avenue cor. Elliptical Road');
  const [barangay, setBarangay] = useState<string>('Central');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (showToast) {
      showToast('Excavation Ground Preparation Permit application saved successfully!');
    }
  };
    ...
    <div className="w-full max-w-5xl mx-auto space-y-4">
      <div className="rounded-xl overflow-hidden border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#0c1629] shadow-xs">
        ...
        <div className="bg-[#0c4366] text-white px-4 py-2.5 text-xs font-bold uppercase tracking-wider">
          Excavation Ground Preparation Permit Application
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-4 sm:p-5 rounded-lg border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[#0b1322] space-y-3.5 shadow-2xs">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-300 block font-normal">
                    Tax dec. no. <span className="text-red-500 font-bold">*</span>
                  </label>
                  <input
                    type="text"
                    value={taxDecNo}
